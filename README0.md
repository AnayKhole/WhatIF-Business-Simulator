...
summary. It also includes two built-in calculators to help the user derive a
realistic growth rate from their own historical data before running the simulation.

Features

- **Strategic decision modelling** — quantifies the growth and cost impact of four
  levers:
  - Hiring additional staff, costed using a user-defined average employee salary
...
- **Growth rate calculators** — a side panel with two tools to help set a realistic
  baseline growth rate:
  - **Historical Revenue Growth Calculator** — enter revenue for several consecutive
    years and get the average year-over-year growth rate, with a full breakdown of
    each period.
  - **CAGR Calculator** — enter a starting value, ending value, and number of years
    to calculate compound annual growth rate.

  Either result can be applied directly to the simulation's baseline growth rate
  with one click.
...
1. The user enters base parameters (revenue, costs, employees, average salary,
...
4. For each selected market scenario, a `MonteCarloSimulator` runs the chosen number
   ...
   simulation/year.
5. Results are aggregated (mean, standard deviation, min, max) per year and scenario,
   then charted and summarised.
6. The simulation runs on a background thread so the UI stays responsive, with a
...
Fill in the base parameters and strategic decisions on the left, optionally use the
growth calculators on the right to set a data-driven growth rate, select which
